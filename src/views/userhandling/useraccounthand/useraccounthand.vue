						...
						<vxe-modal v-model="dialog" title="添加用户账号" width="800" min-width="600" min-height="300" :esc-closable="true" resize
							...
							<template #default>
								<vxe-form :data="userinfo" title-align="right" title-width="100">
									...
									<vxe-form-item field="identity" title="角色" :span="12" :item-render="{}">
										<template #default="{ data }">
											<vxe-select v-model="data.identity" transfer>
												<vxe-option v-for="item in options3" :key="item.value" :value="item.value" :label="item.label"></vxe-option>
											</vxe-select>
										</template>
									</vxe-form-item>
...
import { VxeGridInstance, VxeGridListeners, VxeGridProps, VxePagerEvents, VXETable, VxeTableInstance } from 'vxe-table';
const search = ref('')
const tableData = ref([])
...
const submitData = (data: { account: string; accstatus: any; isloginallowed: any, user_id: number, role_list: number }) => {
	account.value = data.account
	// console.log(data.accstatus)
	updateAccount({ account: data.account, accstatus: data.accstatus, isloginallowed: data.isloginallowed, user_id: data.user_id, role_list: data.role_list }).then((res: any) => {
		if (res.msg == "OK") {
			ElMessage({
				message: "提交成功!",
				type: 'success',
