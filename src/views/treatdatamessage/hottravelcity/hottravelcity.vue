				...
				<el-card class="box-card">
					...
					<div style="margin-top: 10px;">
						<el-table
							...
							<el-table-column prop="dbtime" label="留言时间" />
							<el-table-column prop="dbcontent" label="留言内容" />
		...
		methods:{
			...
			searchMessage(){
				if(this.search=='')
				{
					...
					this.loading=true
					axios({
						url:"http://127.0.0.1:5000/searchhttravelcityemessage",
						method:"POST",
			...
			deleteMessage(row)
			{
				...
				axios({
					url:"http://127.0.0.1:5000/delethottravelcitymess",
					method:"POST",
					data:{"id":row.id,"account":row.account}
				}).then(res=>{
					if(res.data=='ok')
					{
						ElMessage({
